...
| **CRIT-05** | `shaders/compute/nrc_encode_infer.comp:212-214` | **NRC Non-Atomic Storage Image Data Race**: Concurrent query records for the same pixel execute un-synchronized `imageLoad` and `imageStore` on `uAccumImage`, causing high-frequency salt-and-pepper noise and flickering. | Med | High | Substantial Rewrite |
